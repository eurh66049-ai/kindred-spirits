...
const BookSubmissionForm: React.FC<BookSubmissionFormProps> = ({ onSuccess }) => {
  ...
  const uploadFileUnified = async (file: File, bucket: string, folder: string = ''): Promise<string> => {
    ...
    try {
      ...
      try {
        ...
        // إذا فشل XHR، استخدم Supabase SDK كبديل
      ...
      throw new Error(`فشل في رفع ${file.name}: ${error.message || 'خطأ غير معروف'}`);
    }
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      let coverImageUrl = isEdit ? existingFiles.coverImageUrl : null;
      let bookFileUrl = isEdit ? existingFiles.bookFileUrl : null;
          ...
          if (fileData.type === 'cover') {
            ...
            if (fileData.file.type === 'application/pdf' || fileData.file.name.toLowerCase().endsWith('.pdf')) {
              try {
                ...
                const { data: watermarkResult, error: watermarkError } = await supabase.functions.invoke('add-pdf-watermark', {
                  body: {
                    pdfUrl: url,
      ...
      if (isEditApproved && editApprovedId) {
        ...
        const { data: editResult, error: editError } = await supabase.functions.invoke('submit-book-edit', {
          body: {
      ...
      if (!isEdit && !isEditApproved) {
        ...
        await supabase.from('notifications').insert({
          user_id: user.id,
          title: 'تم استلام كتابك! 📚',
          message: `تم استلام كتاب "${formData.title}" (${actualFileSize ? formatFileSize(actualFileSize) : 'حجم غير محدد'}) وبدأت عملية المراجعة. سنقوم بمراجعته بعناية وإشعارك بالنتيجة خلال 72 ساعة.`,
          type: 'info',
