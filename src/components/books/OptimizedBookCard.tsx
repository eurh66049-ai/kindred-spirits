...
const OptimizedBookCard: React.FC<OptimizedBookCardProps> = ({ book, stats, onClick }) => {
  ...
  return (
    <a href={`/book/${bookSlug}`} className="block" onClick={handleClick}>
      <Card 
        className="group cursor-pointer transition-all duration-200 hover:shadow-lg hover:scale-105 bg-card text-card-foreground card-optimized touch-optimized"
      >
