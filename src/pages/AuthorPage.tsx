...
const AuthorPage: React.FC = () => {
  ...
  // استخدام hook للتحقق من حالة طلب المراسلة
  const { getRequestStatus } = useMessageRequests();
  const messageRequestStatus = authorData?.user_id ? getRequestStatus(authorData.user_id) : null;
  const hasPendingSentRequest = messageRequestStatus?.type === 'sent' && messageRequestStatus?.request.status === 'pending';
                  ...
                  <div className="flex items-center justify-center gap-3 mb-5">
                    {shouldShowFollowButton && (
                      <FollowOptionsPopover
                        ...
                        hideText={hasPendingSentRequest}
                      />
                    )}
