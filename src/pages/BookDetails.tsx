...
const BookDetailsContent = () => {
  ...
  const getCanonicalUrl = () => {
    ...
    if (book?.slug) {
      return `${baseUrl}/book/${encodeURIComponent(book.slug)}`;
    }
    // إذا لم يكن هناك slug، نستخدم ID
    return `${baseUrl}/book/${book?.id || id}`;
  };

  const canonicalUrl = book ? getCanonicalUrl() : `https://kotobi.xyz/book/${id}`;

  // URL للمشاركة (يبقى window.location.href للمشاركة الفعلية)
...
const BookDetails = () => {
  ...
  const baseUrl = 'https://kotobi.xyz';
  const canonicalBookUrl = book?.slug 
    ? `${baseUrl}/book/${book.slug}`
    : `${baseUrl}/book/${id}`;
